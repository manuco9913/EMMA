...
- An **interactive MapLibre GL JS 4 map** (OSM raster tiles) with draggable entity markers and per-entity radius circles.
...
3. As a user, I want to open the application and see an interactive map centered on a default location, so that I can immediately begin placing entities.
4. As a user, I want a scenario panel in a left sidebar, so that I can configure scenario parameters without losing sight of the map.
5. As a user, I want the scenario form fields (name, height range, height step, angular resolution, grid cell size, terrain toggle, combination method) rendered from the JSON Schema served by the backend, so that field changes are reflected in the UI without a frontend rebuild.
6. As a user, I want to add between 2 and 10 entities to a scenario, so that I can simulate multiple emitters simultaneously.
7. As a user, I want each entity form (label, position, frequency, power, azimuth, antenna height, radius) rendered from the entity JSON Schema, so that field additions or constraint changes don't require frontend code changes.
8. As a user, I want to place an entity by clicking on the map, so that I can set its position intuitively without typing coordinates.
9. As a user, I want to drag an entity marker on the map to update its position, so that I can adjust placement quickly.
10. As a user, I want lat/lon coordinate fields in the entity form to stay in bidirectional sync with the map marker, so that I can use either input method interchangeably.
11. As a user, I want each entity's radius visualized as a filled circle on the map, so that I can see its area of interest.
12. As a user, I want the radius circle to update in real time as I change the radius field, so that I can see the effect immediately.
13. As a user, I want numeric fields to show a unit suffix (e.g. MHz, dBm, m, km, °), so that I know what unit to enter.
14. As a user, I want fields with `x-showIf` conditions to hide or show dynamically based on other field values, so that only relevant fields are visible.
15. As a user, I want `value-or-file` fields (frequency, power) to let me toggle between an inline value and a file path, so that I can supply either a scalar or a per-range lookup table.
16. As a user, I want form validation (type checks, min/max constraints) to run on blur and on submit, so that I get feedback without waiting until submission.
17. As a user, I want clear inline error messages next to each invalid field, so that I know exactly what to fix.
18. As a user, I want to submit a scenario and immediately see a job progress indicator, so that I know the backend has received my request and is working.
19. As a user, I want job progress to stream in real time via SSE without polling, so that the UI stays responsive during computation.
20. As a user, I want the heatmap overlay to appear automatically when the job completes, so that I don't need to manually trigger rendering.
21. As a user, I want a height slider that lets me scrub through height levels of the propagation output, so that I can explore signal strength at different altitudes.
22. As a user, I want to type a specific height value numerically in addition to using the slider, so that I can jump to a precise altitude.
23. As a user, I want adjacent height levels (±2) prefetched in the background, so that scrubbing feels smooth without waiting for each level to load.
24. As a user, I want the heatmap rendered as a GPU-accelerated R32F texture using a custom Deck.gl layer, so that large grids render at interactive frame rates.
25. As a user, I want null cells (outside any entity radius) to render as transparent, so that only the valid propagation area is shown.
26. As a user, I want the heatmap positioned correctly on the map using the bounding box from the slice header, so that I can correlate signal strength with terrain features.
27. As a user, I want to save a completed run by giving it a name, so that I can preserve results before re-running with different parameters.
28. As a user, I want a confirmation dialog before an unsaved run is deleted, so that I don't accidentally lose results.
29. As a user, I want to re-run a scenario after modifying parameters, so that I can iterate on configurations.
30. As a developer, I want `evaluateShowIf` to be a pure function with no side effects, so that it can be unit tested without a DOM or form context.
31. As a developer, I want `SliceParser` to be a pure function that takes a raw `ArrayBuffer` and returns a typed result, so that it can be unit tested without network or browser APIs.
32. As a developer, I want `SchemaFormRenderer` to accept a schema object and react-hook-form `control` as props, so that it can be tested in isolation with a fixture schema.
33. As a developer, I want Zustand stores instantiated via factory functions, so that each test gets a fresh store without global state leaking.
34. As a developer, I want integration tests covering the full flow (form fill → validation → POST → SSE → heatmap render), so that the golden path is verified without mocking internal implementation details.
...
- `x-widget` values: `coordinate`, `range`, `matrix`, `value-or-file`. Fields without `x-widget` fall back to type-based rendering (`number` → `NumberField`, `string` → `StringField`, `string+enum` → `EnumField`, `boolean` → `BooleanField`).
- `x-showIf` supports `and`/`or` top-level operators with `{ field, op, value }` entries. Ops: `eq`, `neq`, `gt`, `gte`, `lt`, `lte`. Field references are scoped to the same object.
- `x-unit` is display-only — a suffix shown next to the input, not included in the submitted value.

### Modules

**`evaluateShowIf(condition, formValues) → boolean`** — Pure function. No React dependencies. Takes the `x-showIf` object and current form values; returns whether the field should be shown.
...
**`SchemaFormRenderer`** — Generic React component. Props: `schema`, `control`, `watch`. Iterates `schema.properties` in key order, evaluates `x-showIf` per field, mounts the correct field renderer. No hardcoded field names.
...
**Map component** — MapLibre GL JS 4 via react-map-gl v7. OSM raster XYZ tile source. Entity markers as `<Marker>` (draggable). Radius circles as a GeoJSON `fill` layer, updated reactively from `mapStore`.
...
- `SchemaFormRenderer` + field renderers — unit tests with React Testing Library. Render with fixture schemas; assert correct field types rendered, `x-showIf` hides/shows fields live, validation errors appear on submit, `value-or-file` toggle works.
...
- PMTiles vector tile serving — Phase 2+
...
- OSM raster tiles are a Phase 1 simplification. The PMTiles migration expected in Phase 2 must not require changes to the heatmap layer or form system.
